...
const DEFAULT_IMMUNITY_CONFIG = {
  ...
  firstContentStart: 0.25,
  // First content appears at 25% of scroll
  firstContentEnd: 0.5,
  // First content disappears at 50% of scroll
  secondContentStart: 0.5,
  // Second content appears at 50% of scroll
  secondContentEnd: 0.75,
  // Second content disappears at 75% of scroll
  contentHideEnd: 1,
  // Content wrapper hides at end of section
...
function initImmunitySectionAnimation(pageSelector, customConfig = {}) {
  ...
  ScrollTrigger.create({
    ...
    start: "top+=20% center",
    // Appears when user scrolls 25% into the section
    end: "top+=35% center",
    // Disappears when user scrolls 50% into the section
    markers: cfg.debug,
    onEnter: () => {
  ...
  ScrollTrigger.create({
    trigger: immunitySection,
    start: "top+=70% center",
    // Appears when user scrolls 50% into the section
    end: "top+=80% center",
    // Disappears when user scrolls 75% into the section
    markers: cfg.debug,
    onEnter: () => {
...
const DEFAULT_WELLNESS_CONFIG = {
  ...
  textAppearOffset: 200,
  // Text appears 200px after section top
...
function initWellnessSectionAnimation(pageSelector, customConfig = {}) {
  ...
  gsap.set(wellnessTitle, {
    opacity: 0
  });
  gsap.set(wellnessDescription, {
    opacity: 0
  });
  gsap.set(wellnessContent, {
    opacity: 0
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  waitForGSAPAndInitWellness(".accommodation-template", {
    debug: false,
    // See the markers
    textAppearOffset: 1100,
    // Change when text appears
    textDisappearOffset: -700,
    // Change when text disappears
